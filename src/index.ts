...
export default class Chooser {
   ...
   */
  static chooseWeightedIndex = (weights: number[], seed: any = Math.random(), defaultWeight = 1): number => {
    ...
    const seededRandFunc = new (SeedRandom as any)(seed);
    // Select our value.
  ...
  static chooseWeightedObject = (
    ...
    const weights: number[] = arrayOfObjects.map((currItem: any) => {
    ...
    const chosenIndex: number = Chooser.chooseWeightedIndex(weights, seed, defaultWeight);
