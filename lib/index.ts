 ...
 */
export default function chooseWeightedIndex(
  weights: number[],
  ...
  let ranges: number[] = weights.map(
    weight =>
      (cumulative +=
        weight && typeof weight === "number" ? Math.abs(weight) : defaultWeight)
  );
 ...
 * @returns The chosen object, or `null` if the array was empty or all weights were `0`.
 */
export function chooseWeightedObject(
  ...
  let chosenIndex: number = chooseWeightedIndex(weights);
